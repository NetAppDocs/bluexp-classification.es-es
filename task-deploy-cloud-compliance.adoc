...
[role="quick-margin-para"]
Los primeros 500 TB de datos que analiza la clasificación de BlueXP en BlueXP son gratuitos durante 30 días. Debe haber una suscripción a BlueXP a través de su plataforma de proveedores de cloud o una licencia BYOL de NetApp para continuar analizando los datos después de ese punto.
...
Los sistemas ONTAP en las instalaciones, los recursos compartidos de archivos que no son de NetApp, el almacenamiento de objetos S3 genérico y las bases de datos se pueden analizar al utilizar cualquiera de estos conectores en la nube.
