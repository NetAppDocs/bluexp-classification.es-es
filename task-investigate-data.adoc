...
NOTA: El lanzamiento de diciembre de 2023 (versión 1.26.6) eliminó temporalmente la opción de calcular el número de datos de información personal identificable (PII) por directorios. 
...
* Puede mover o copiar el archivo a cualquier recurso compartido NFS. Consulte link:task-managing-highlights.html#move-source-files-to-an-nfs-share["Mover archivos de origen a un recurso compartido NFS"] y.. link:task-managing-highlights.html#copy-source-files["Copiando archivos de origen a un recurso compartido NFS"] para obtener más detalles.
* Puede eliminar el archivo. Consulte link:task-managing-highlights.html#delete-source-files["Eliminando archivos de origen"] para obtener más detalles.
* Puede asignar un estado determinado al archivo. Consulte link:task-org-private-data.html#apply-tags-to-manage-your-scanned-files["Aplicación de etiquetas"] para obtener más detalles.
* Puede asignar el archivo a un usuario de BlueXP para que sea responsable de las acciones de seguimiento que se deban realizar en el archivo. Consulte link:task-org-private-data.html#assign-users-to-manage-certain-files["Asignar usuarios a un archivo"] para obtener más detalles.
* Si has integrado etiquetas AIP con la clasificación de BlueXP, puedes asignar una etiqueta a este archivo o cambiarla a otra, si ya existe alguna. Consulte link:task-org-private-data.html#assign-aip-labels-manually["Asignación manual de etiquetas AIP"] para obtener más detalles.
