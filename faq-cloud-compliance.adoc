...
https://["Obtén más información sobre los casos de uso para la clasificación de BlueXP"^].
...
Sí. Puede gestionar etiquetas AIP en los archivos a los que está analizando la clasificación de BlueXP si ya se ha suscrito https://["Protección de información de Azure (AIP)"^]. Puede ver las etiquetas que ya están asignadas a los archivos, agregar etiquetas a los archivos y cambiar las etiquetas existentes.
...
La clasificación de BlueXP solo puede analizar datos de orígenes de datos locales al sitio on-premises. En este momento, la clasificación de BlueXP puede analizar las siguientes fuentes de datos locales en un sitio «oscuro»:
...
Consulte https://["precios"^] para obtener más detalles.
...
Por tanto, si tiene datos en muchas de estas ubicaciones, es posible que tenga que utilizarlos https://["Múltiples conectores"^].



=== ¿Puedo desplegar el conector en mi propio host?

Sí. Puede hacerlo https://["Ponga en marcha el conector en las instalaciones"^] En un host Linux en su red o en un host del cloud. Si tienes pensado implementar la clasificación de BlueXP en las instalaciones, es posible que desee instalar el conector también en las instalaciones, pero no es necesario.



=== ¿Qué pasa con sitios seguros sin acceso a Internet?

Sí, también es compatible. Puede hacerlo https://["Implemente el conector en un host Linux local que no tenga acceso a Internet"^]. A continuación, puedes detectar clústeres de ONTAP on-premises y otras fuentes de datos locales y analizar los datos mediante la clasificación de BlueXP.
