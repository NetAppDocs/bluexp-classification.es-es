...
Si su licencia caduca o ha alcanzado el límite BYOL, Data Sense continúa ejecutándose, pero el acceso a los paneles está bloqueado para que no pueda ver información sobre ninguno de los datos analizados. Solo la página _Configuration_ está disponible en caso de que se desee reducir la cantidad de volúmenes que se van a analizar para lograr que su uso de capacidad esté dentro del límite de licencia.
...
NOTE: Si la cuenta que está utilizando tiene una licencia BYOL y una suscripción a PAYGO, Data Sense _no pasará a la suscripción a PAYGO cuando caduque la licencia BYOL. Debe renovar la licencia de BYOL.
